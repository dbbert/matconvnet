...
%     end
        ...
        switch opts.type
            case 'discriminator'
                outputs = add_blockNoBatchNorm(net, netOpts, 'block1', inputs, 5, 5, netOpts.nChns, 8, 2, 2);
                outputs = add_blockNoBatchNorm(net, netOpts, 'block2', outputs, 5, 5, 8, 16, 2, 2);
                outputs = add_blockNoBatchNorm(net, netOpts, 'block3', outputs, 5, 5, 16, 32, 2, 2);
                outputs = add_conv(net, netOpts, 'block4', outputs, 4, 4, 32, 2, 1, 0);
                net.addLayer('loss', dagnn.Loss('loss', 'softmaxlog'), [outputs, {'label'}], 'objective') ;
                net.addLayer('error', dagnn.Loss('loss', 'classerror'), [outputs, {'label'}], 'error') ;
            case 'generator'
                ...
                outputs = add_deblock(net, netOpts, 'deblock3', outputs, 4, 4, 128, 64, 2, 1);
                outputs = add_deblock(net, netOpts, 'deblock2', outputs, 4, 4, 64, 32, 2, 1);
                outputs = add_deconv(net, netOpts, 'deblock1', outputs, 4, 4, 32, netOpts.nChns, 2, 1);
...
%         switch opts.type
%             case 'discriminator'
%                 outputs = add_blockNoBatchNorm(net, netOpts, 'block1', inputs, 3, 3, netOpts.nChns, 16, 1, 1);
%                 outputs = add_blockNoBatchNorm(net, netOpts, 'block1b', outputs, 3, 3, 16, 16, 2, 1);
% %                 outputs = add_pool(net, netOpts, 'block1', outputs, 2, 2, 2, 0);
%                 outputs = add_blockNoBatchNorm(net, netOpts, 'block2', outputs, 3, 3, 16, 32, 1, 1);
%                 outputs = add_blockNoBatchNorm(net, netOpts, 'block2b', outputs, 3, 3, 32, 32, 2, 1);
% %                 outputs = add_pool(net, netOpts, 'block2', outputs, 2, 2, 2, 0);
%                 outputs = add_blockNoBatchNorm(net, netOpts, 'block3', outputs, 3, 3, 32, 64, 1, 1);
%                 outputs = add_blockNoBatchNorm(net, netOpts, 'block3b', outputs, 3, 3, 64, 64, 2, 1);
%                 outputs = add_blockNoBatchNorm(net, netOpts, 'block4', outputs, 4, 4, 64, 2, 1, 0);
%                 net.addLayer('loss', dagnn.Loss('loss', 'softmaxlog'), [outputs, {'label'}], 'objective') ;
%                 net.addLayer('error', dagnn.Loss('loss', 'classerror'), [outputs, {'label'}], 'error') ;
%             case 'generator'
%     %             outputs = add_norm(net, netOpts, 'deblockNorm', inputs, );
...
%                 outputs = add_deblock2(net, netOpts, {'deblock3','deblock3b'}, outputs, 3, 3, 128, 64, 1, 1, 0);
%                 outputs = add_upsample(net, netOpts, 'unpool2', outputs, 64, true);
%                 outputs = add_deblock2(net, netOpts, {'deblock2','deblock2b'}, outputs, 3, 3, 64, 32, 1, 1, 0);
%                 outputs = add_upsample(net, netOpts, 'unpool1', outputs, 32, true);
%                 outputs = add_deblock(net, netOpts, 'deblock1b', outputs, 3, 3, 32, 32, 1, 1);
%                 outputs = add_deblockSigmoid(net, netOpts, 'deblock1', outputs, 3, 3, 32, netOpts.nChns, 1, 1);
%             case 'encoder'
%                 outputs = add_block(net, netOpts, 'block1', inputs, 3, 3, netOpts.nChns, 8, 2, 1);
%     %             outputs = add_conv(net, netOpts, 'block1', inputs, 5, 5, netOpts.nChns, 8, 2, 2);
%     %             outputs = add_relu(net, netOpts, 'block1', outputs, 0.2);
% 
%                 outputs = add_block(net, netOpts, 'block2', outputs, 3, 3, 8, 16, 2, 1);
%                 outputs = add_block(net, netOpts, 'block3', outputs, 3, 3, 16, 32, 2, 1);
%     %             outputs = add_block(net, netOpts, 'block4', outputs, 4, 4, 128, 2, 1, 0);
%                 outputs = add_conv(net, netOpts, 'block4', outputs, 4, 4, 32, vectorSize, 1, 0);
%                 outputs = add_normalize(net, netOpts, 'normalize', outputs);
%                 net.addLayer('loss', dagnn.Loss('loss', 'euclidean'), [outputs, {'label'}], 'objective') ;
%         end
