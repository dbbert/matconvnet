function stats = trainNoise(varargin)
...
opts.baseDir = 'dataNoise';
opts.dataset = 'mnist';
% opts.dataset = 'cifar';
opts.imageSize = [32 32 1] ;
opts.seed = 5;
opts.modelType = 'test' ;
opts.learningRate = logspace(-2, -4, 100) ;
opts.batchSize = 128 ;
opts.nClasses = 8 ;
...
% generator
opts.imageSize = [32 32 1] ;
generatorNet = getNetNoise('type', 'generator', 'imageSize', opts.imageSize);
...
discriminatorNet = getNetNoise('type', 'discriminator', 'imageSize', opts.imageSize);
...
% Launch SGD
[~, stats] = cnn_train_dag_adversarialNoise(generatorNet, discriminatorNet, imdb, getBatchWrapper(bopts), opts.train, ...
  'train', train, ...
  'val', val) ;
