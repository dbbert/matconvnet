...
%     switch opts.type
%         case 'discriminator'
%             outputs = add_block(net, netOpts, 'block0', inputs, 3, 3, netOpts.nChns, 16, 1, 1);
%             outputs = add_block(net, netOpts, 'block0b', outputs, 3, 3, 16, 32, 2, 1);
%             outputs = add_block(net, netOpts, 'block1', outputs, 8, 8, 32, vectorSize, 1, 0);
%             outputs = add_conv(net, netOpts, 'block2', outputs, 1, 1, vectorSize, vectorSize, 1, 0);
% %             outputsNormalized = add_normalize(net, netOpts, 'normalize', outputs);
%             net.addLayer('euclidean', dagnn.Loss('loss', 'euclidean'), [outputs, {'embedding'}], 'euclidean') ;
%             outputs = add_block(net, netOpts, 'block5', outputs, 1, 1, vectorSize, 8, 1, 0);
%             outputs = add_conv(net, netOpts, 'block6', outputs, 1, 1, 8, 2, 1, 0);
...
%             outputs = add_deblock(net, netOpts, 'deblock3', inputs, 8, 8, vectorSize, 32, 1, 0);
%             outputs = add_upsample(net, netOpts, 'unpool3', outputs, 32, true);
%             outputs = add_deconv(net, netOpts, 'deblock4', outputs, 3, 3, 32, netOpts.nChns, 1, 1);
% %             outputs = add_batchNorm(net, netOpts, 'deblock4', outputs, netOpts.nChns);    
